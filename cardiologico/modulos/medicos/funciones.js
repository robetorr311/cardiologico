...
function newMedico(){
	var ajax=nuevoAjax();
	var divVerRegistro=document.getElementById("medicos");
	ajax.open("POST","newMedico.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send(null);
	ajax.onreadystatechange=function()
	{
...
function guardarMedicos(){
	...
	var divmedicos=document.getElementById("medicos");
	var med=0;
	if(!cedula) med=1;
	if(!nombre) med=1;
	if(!edad) med=1;
	...
	if(med==1){
		...
		if(!loggin) alert("Falto ingresar el nombre de usuario");
		if(!password) alert("Falto ingresar la clave de acceso al sistema");		
		...
		ajax.open("POST","guardarMedicos.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("cedula="+cedula+"&nombre="+nombre+"&edad="+edad+"&sexo="+sexo+"&telefono="+telefono+"&correo="+correo+"&direccion="+direccion+"&establecimiento="+establecimiento+"&sector="+sector+"&loggin="+loggin+"&password="+password);
		var divmedicos=document.getElementById("medicos");
		ajax.onreadystatechange=function()
		{
			if (ajax.readyState==4)
			{
				var respuesta=ajax.responseText;
				divmedicos.innerHTML=respuesta;
			}
		}
	}
}
function updateMedico(){
	...
	if(med==1){
		...
		ajax.open("POST","updateMedico.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("id="+id+"&cedula="+cedula+"&nombre="+nombre+"&edad="+edad+"&sexo="+sexo+"&telefono="+telefono+"&correo="+correo+"&direccion="+direccion+"&establecimiento="+establecimiento+"&sector="+sector);
		var divmedicos=document.getElementById("medicos");
		ajax.onreadystatechange=function()
		{
			if (ajax.readyState==4)
