...
window.setInterval("actualizarRespuestas()", 2000);
window.setInterval("actualizarMensajes()", 2000);
window.setInterval("botonguardar()", 2000);
function preCarga(){
	imagenes=new Array();
...
function CerrarSesion(){
	...
	ajax.open("POST","cerrarsession.php", true);
	...
	ajax.onreadystatechange=function()
		{
			if (ajax.readyState==4)
			{
				...
				divsession.innerHTML=respuesta;
			}
		}
}
...
function SubirCTK(id){
	var punto =".";
	muestraMensaje(punto);
	datosModal=window.showModalDialog('subirCTK.php',id,'dialogWidth:750px;dialogHeight:400px;dialogLeft:10;dialogTop:200;status:0;unadorned:yes;toolbar:0;help:no');
	if (datosModal!=null){
		var ajax=nuevoAjax();
...
function SubirImagen(){
	...
	if (datosModal!=null){
		var hecg=datosModal[0];
		var hexamen=datosModal[1];
		var ajax=nuevoAjax();
		var divbotones=document.getElementById("botones");
		ajax.open("POST","botonesImagen.php", true);
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("hexamen="+hexamen);
		ajax.onreadystatechange=function()
		{
			if (ajax.readyState==4)
			{
...
function agregarPacientes(){
	...
	if(alerta==1){
		...
		ajax.onreadystatechange=function()
		{
			if (ajax.readyState==4)
			{
				document.getElementById("formulario").diagnostico.disabled=false;
				document.getElementById("formulario").anexar.disabled=false;
				var respuesta=ajax.responseText;
...
function editarPacientes(){
	...
	ajax.onreadystatechange=function()
	{
		...
		{
			var respuesta=ajax.responseText;
			document.getElementById("formulario").diagnostico.disabled=true;
			document.getElementById("formulario").anexar.disabled=true;					
			divpaciente.innerHTML=respuesta;
		}
	}
}
...
function paciente(){
	...
	ajax.onreadystatechange=function()
	{
		if (ajax.readyState==4)
		{
			...
			if(respuesta=="0") {
				var ajaxn=nuevoAjax();
				ajaxn.open("POST","pacienteno.php", true);
				ajaxn.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
				ajaxn.send("cedula="+cedula);
				ajaxn.onreadystatechange=function()
				{
					if (ajaxn.readyState==4)
					{
						var respuestano=ajaxn.responseText;
						document.getElementById("formulario").diagnostico.disabled=true;
						document.getElementById("formulario").anexar.disabled=true;						
						divpaciente.innerHTML=respuestano;
					}
				}					
			}
			else{
				var ajaxs=nuevoAjax();
				ajaxs.open("POST","pacientesi.php", true);
				ajaxs.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
				ajaxs.send("cedula="+cedula);
				ajaxs.onreadystatechange=function()
				{
					if (ajaxs.readyState==4)
					{
						var respuestasi=ajaxs.responseText;
						document.getElementById("formulario").diagnostico.disabled=false;
						document.getElementById("formulario").anexar.disabled=false;						
						divpaciente.innerHTML=respuestasi;					
					}
				}				
			}
...
function actualizarRespuestas(){
	var ajax=nuevoAjax();
	//var usuario=document.getElementById("formulario").usuario.value;
...
function actualizarMensajes(){
	...
	var divmodalResp=document.getElementById("mensajes");
	ajax.open("POST","verMensajes.php", true);
	ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	ajax.send("husuario="+husuario);
	ajax.onreadystatechange=function()
	{
		if (ajax.readyState==4)
		{
...
function validaForm(){
	...
	var husuario=document.getElementById("formulario").husuario.value;	
	var idpaciente=document.getElementById("formulario").idpaciente.value;
	var hexamen=document.getElementById("formulario").hexamen.value;
 	...
 	if (error==1)
	{
		...
		if (!hexamen) alert("Falta Ingresar los examenes fisicos");
	...
	{ 
		...
		ajax.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		ajax.send("usuario="+usuario+"&husuario="+husuario+"&idpaciente="+idpaciente+"&hexamen="+hexamen+"&diagnostico="+diagnostico);
		ajax.onreadystatechange=function()
		{
			if (ajax.readyState==4)
			{
...
function botonguardar(){
	...
	if (!diagnostico) error=1;
 	if (error==1){
		...
		document.getElementById("formulario").guardar.disabled=false;		
	}
}